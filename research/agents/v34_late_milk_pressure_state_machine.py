...
_LATE_MILK_STATE = {0: {}, 1: {}}


def _opponent_animal_mix(obs):
    seat = _seat(obs)
    ...
    return sum(counts.values()), counts.get("SHEEP", 0)


def _late_milk_switch_active(obs, step):
    seat = _seat(obs)
    state = _LATE_MILK_STATE[seat]
    ...
    if step == 361:
        ...
        total, sheep = _opponent_animal_mix(obs)
        state["active"] = (
            milk_price < 130
            and total >= 12
            and sheep != 6
...
def _apply_late_milk_switch(obs, action, step):
    if not _late_milk_switch_active(obs, step):
        return action
    if step == 361:
        ...
        for unit_action in unit_actions:
            if unit_action and unit_action[0] == "PLACE" and unit_action[1] == "COW":
                unit_action[1] = "SHEEP"
    return action
_PRICE_FLOOR = 1
_DEMAND_ALPHA = 0.25
...
def agent(obs, configuration=None):
    try:
        ...
        return _align_hands(_apply_late_milk_switch(obs, action, step), obs)
    except Exception:
