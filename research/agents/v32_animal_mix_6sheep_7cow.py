...
# V32 single-variable route mutation: two existing COW slots become SHEEP.
for _action_step in (192,):
    for _order in _REBALANCE_ACTIONS[_action_step].get("market", []):
        if _order and _order[0] == "BUY_ANIMAL" and _order[1] == "COW":
            _order[1] = "SHEEP"
for _action_step in (198, 204):
    for _unit_actions in ([_REBALANCE_ACTIONS[_action_step].get("farmer", [])]
                          + _REBALANCE_ACTIONS[_action_step].get("hands", [])):
        if _unit_actions and _unit_actions[0] == "PLACE" and _unit_actions[1] == "COW":
            _unit_actions[1] = "SHEEP"
_PRICE_FLOOR = 1
_DEMAND_ALPHA = 0.25
_MARKET_PARAMS = {
