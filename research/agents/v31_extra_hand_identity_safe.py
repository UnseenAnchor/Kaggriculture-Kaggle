...
for _day in range(30):
    _day_steps = range(_day * 24, min((_day + 1) * 24, len(_REBALANCE_ACTIONS)))
    for _candidate_step in reversed(tuple(_day_steps)):
        _market = _REBALANCE_ACTIONS[_candidate_step].get("market", [])
        if any(order and order[0] == "HIRE" for order in _market) and len(_market) < 10:
            _EXTRA_HIRE_STEPS[_day] = _candidate_step
            break


def _extra_hand_action(obs, action, step):
    action = _align_hands(action, obs)
    day = int(step) // 24
    hire_step = _EXTRA_HIRE_STEPS.get(day)
    if hire_step is None or int(step) <= hire_step:
        return action
    ...
    for y, row in enumerate(list(_get(farm, "tiles", []) or [])):
        for x, tile in enumerate(list(row or [])):
            if not isinstance(tile, dict) or not tile.get("animal"):
                continue
            available = int(tile.get("fertilizer_available", 0) or 0)
            if available > 0:
                targets.append((x, y, available))
    if not targets:
        return action
    occupied = {
        (int(position[0]), int(position[1]))
        for actor, position in enumerate(hands)
        if actor != index and isinstance(position, (list, tuple)) and len(position) >= 2
    }
    x, y = int(hands[index][0]), int(hands[index][1])
    targets = [target for target in targets if (target[0], target[1]) not in occupied]
    if not targets:
        return action
    ...
    tx, ty, _ = targets[0]
    if (x, y) == (tx, ty):
        action["hands"][index] = ["COLLECT_FERTILIZER"]
    elif x < tx:
        action["hands"][index] = ["EAST"]
    elif x > tx:
        action["hands"][index] = ["WEST"]
    elif y < ty:
        action["hands"][index] = ["SOUTH"]
    else:
        action["hands"][index] = ["NORTH"]
    return action
...
def agent(obs, configuration=None):
    try:
        ...
        if _regime(configuration) == "rebalance" and step in _EXTRA_HIRE_STEPS.values():
            action["market"] = list(action.get("market") or [])
            if len(action["market"]) < 10:
                action["market"].append(["HIRE"])
        return _extra_hand_action(obs, _align_hands(action, obs), step)
    except Exception:
        farm = _farm(obs, _seat(obs))
        return {
            "farmer": ["PASS"],
