...
# V37: do not promote WHEAT into an earlier sell slot while the opponent is
# cash-starved.  Early wheat supply lowers the feed price the opponent pays;
# preserve the tape's wheat slot while retaining v27's ordering for all other
# products.  This changes market ordering only, never production or hand work.
_V37_HOST_AGENT = agent
_V37_CASH_LIMIT = 200.0


def _v37_wheat_cash_guard(obs, action):
    farms = _get(obs, "farms", []) or []
    player = int(_get(obs, "player", 0) or 0)
    if len(farms) < 2:
        return action
    opponent = farms[1 - player] or {}
    if float(_get(opponent, "money", 0) or 0) >= _V37_CASH_LIMIT:
        return action
    step = min(max(0, int(_get(obs, "step", 0) or 0)), len(_LEGACY_ACTIONS) - 1)
    original = _LEGACY_ACTIONS[step].get("market", []) or []
    wheat_slots = [
        index for index, order in enumerate(original)
        if _is_sell(order) and order[1] == "WHEAT"
    ]
    if not wheat_slots:
        return action
    market = list(action.get("market") or [])
    sell_indices = [index for index, order in enumerate(original) if _is_sell(order)]
    wheat = [order for order in market if _is_sell(order) and order[1] == "WHEAT"]
    others = [order for order in market if _is_sell(order) and order[1] != "WHEAT"]
    if not wheat:
        return action
    for index in wheat_slots:
        if index < len(market) and wheat:
            market[index] = wheat.pop(0)
    queue = iter(others)
    for index in sell_indices:
        if index in wheat_slots:
            continue
        if index < len(market):
            replacement = next(queue, None)
            if replacement is not None:
                market[index] = replacement
    action["market"] = market
    return action


def agent(obs, configuration=None):
    action = _V37_HOST_AGENT(obs, configuration)
    try:
        return _v37_wheat_cash_guard(obs, action)
    except Exception:
        return action


def v37_submission_agent(obs, configuration=None):
    return agent(obs, configuration)
