...
_PRESSURE_CROP_STATE = {0: {}, 1: {}}


def _opponent_animal_total(obs):
    seat = _seat(obs)
    ...
    return sum(
        1
        for row in list(_get(farms[opponent], "tiles", []) or [])
        for tile in list(row or [])
        if isinstance(tile, dict) and tile.get("animal")
    )


def _pressure_crop_active(obs, step):
    seat = _seat(obs)
    state = _PRESSURE_CROP_STATE[seat]
    ...
    if step == 192:
        ...
        milk_price = float(_get(prices, "MILK", 10_000) or 10_000)
        opponent_animals = _opponent_animal_total(obs)
        state["active"] = (
            milk_price <= 190
            and 9 <= opponent_animals <= 10
        )
    return bool(state.get("active", False))


def _apply_pressure_crop_switch(obs, action, step):
    if not _pressure_crop_active(obs, step) or step < 192:
        return action
    for order in action.get("market", []):
        if len(order) >= 2 and order[0] in ("BUY_SEED", "SELL") and order[1] == "STRAWBERRY":
            order[1] = "WHEAT"
    unit_actions = [action.get("farmer", []), *action.get("hands", [])]
    for unit_action in unit_actions:
        if len(unit_action) >= 2 and unit_action[0] == "PLANT" and unit_action[1] == "STRAWBERRY":
            unit_action[1] = "WHEAT"
    return action
_PRICE_FLOOR = 1
_DEMAND_ALPHA = 0.25
...
def agent(obs, configuration=None):
    try:
        ...
        return _align_hands(_apply_pressure_crop_switch(obs, action, step), obs)
    except Exception:
