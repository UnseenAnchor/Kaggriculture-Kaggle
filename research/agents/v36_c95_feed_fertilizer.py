...
# V36: transplant only the externally validated C94 market overlay.  The v27
# field/hand tape remains untouched; this is intentionally not a route relay.
_C95_HOST_AGENT = agent
_C95_DEBT = {0: {}, 1: {}}
_C95_LAST_STEP = {0: -1, 1: -1}
_C95_SPLIT_CAP = 5
_C95_SPLIT_START = 120
_C95_SPLIT_STOP = 715


def _c95_opening(action, step):
    if step != 0:
        return action
    market = [list(order) for order in (action.get("market") or [])]
    feed = [
    ...
    if not feed:
        return action
    rest = [order for order in market if order not in feed]
    action["market"] = [["BUY_PRODUCT", "WHEAT", 5], *rest][:10]
    return action


def _c95_seat_reset(obs, step):
    seat = _seat(obs)
    if step == 0 or step <= _C95_LAST_STEP[seat]:
        _C95_DEBT[seat] = {}
    _C95_LAST_STEP[seat] = step
    return seat


def _c95_repay(action, due):
    if not due:
        return action
    out = []
...
def _c95_fertilizer_split(action, obs, step, debt_schedule):
    if step < _C95_SPLIT_START or step >= _C95_SPLIT_STOP:
        return action
    future_step = step + 1
    if future_step >= len(_LEGACY_ACTIONS):
        return action
    planned = sum(
        max(0, int(order[2] or 0))
        for order in _LEGACY_ACTIONS[future_step].get("market", [])
    ...
    available = max(0, int(_get(shed, "FERTILIZER", 0) or 0) - committed)
    quantity = min(planned, _C95_SPLIT_CAP, available)
    if quantity <= 0:
        return action
    market.append(["SELL", "FERTILIZER", quantity])
    action["market"] = market[:10]
    debt_schedule[future_step] = debt_schedule.get(future_step, 0) + quantity
    return action


def agent(obs, configuration=None):
    action = _C95_HOST_AGENT(obs, configuration)
    try:
        step = int(_get(obs, "step", 0) or 0)
        seat = _c95_seat_reset(obs, step)
        debt_schedule = _C95_DEBT[seat]
        due = debt_schedule.pop(step, 0)
        action = _c95_repay(action, due)
        action = _c95_opening(action, step)
        return _c95_fertilizer_split(action, obs, step, debt_schedule)
    except Exception:
        return action


def v36_submission_agent(obs, configuration=None):
    return agent(obs, configuration)
