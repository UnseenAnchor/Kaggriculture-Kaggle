...
# V38: when the fixed route is cash-tight, protect feed purchases from being
# consumed by optional seed/animal/hire orders.  SELL positions are untouched;
# only non-SELL market orders are reordered, and only when projected cash leaves
# too little room for the feed buy plus a working-capital reserve.
_V38_HOST_AGENT = agent
_V38_RESERVE = 50.0
_V38_FEED_RESERVE = 30.0
_V38_SEED_COST = {
    "WHEAT": 10.0,
    "CARROT": 20.0,
    "TOMATO": 50.0,
...
_V38_ANIMAL_COST = {"GOOSE": 300.0, "COW": 400.0, "SHEEP": 500.0}


def _v38_order_cost(obs, order):
    ...
    if op == "BUY_PRODUCT" and len(order) >= 3:
        ...
        return float(_get(prices, order[1], 50) or 50) * quantity
    if op == "BUY_SEED" and len(order) >= 3:
        return _V38_SEED_COST.get(order[1], 0.0) * quantity
    if op == "BUY_ANIMAL" and len(order) >= 3:
        return _V38_ANIMAL_COST.get(order[1], 0.0) * quantity
    if op == "BUY_LAND":
        return 4000.0
    if op == "HIRE":
        return 50.0
...
def _v38_feed_first(obs, action):
    market = list(action.get("market") or [])
    feed = [
    ...
    feed_cost = sum(_v38_order_cost(obs, order) for order in feed)
    nonfeed = [order for order in market if order not in feed and not _is_sell(order)]
    nonfeed_cost = sum(_v38_order_cost(obs, order) for order in nonfeed)
    if money - nonfeed_cost >= feed_cost + _V38_RESERVE:
        return action
    non_sell_indices = [index for index, order in enumerate(market) if not _is_sell(order)]
    queue = iter(feed + nonfeed)
    for index in non_sell_indices:
        market[index] = next(queue)
    action["market"] = market
    return action


def agent(obs, configuration=None):
    action = _V38_HOST_AGENT(obs, configuration)
    try:
        return _v38_feed_first(obs, action)
    except Exception:
        return action


def v38_submission_agent(obs, configuration=None):
    return agent(obs, configuration)
