...
_PRESSURE_ITEMS = ("WHEAT", "FERTILIZER", "MILK", "WOOL")
_PRESSURE_LIMITS = {"WHEAT": 12, "FERTILIZER": 12, "MILK": 6, "WOOL": 6}
_PRESSURE_MIN_PRICES = {"WHEAT": 30, "FERTILIZER": 60, "MILK": 180, "WOOL": 140}


def _public_animal_pressure(obs):
    seat = _seat(obs)
    farms = list(_get(obs, "farms", []) or [])
    opponent = 1 - seat
...
def _pressure_liquidation(obs, action, step, configuration):
    if _regime(configuration) != "rebalance" or step < 191 or step % 24 != 23:
        return action
    if _public_animal_pressure(obs) < 12:
        return action
    action = _copy_action(action)
    market = list(action.get("market") or [])
    if len(market) >= 10:
        return action
    existing = {
        str(order[1]) for order in market
        if _is_sell(order)
    }
    ...
    candidates = []
    for item in _PRESSURE_ITEMS:
        if item in existing:
            continue
        quantity = int(_get(shed, item, 0) or 0)
        price = int(_get(prices, item, 0) or 0)
        if quantity > 0 and price >= _PRESSURE_MIN_PRICES[item]:
            candidates.append((price * quantity, item, quantity))
    if not candidates:
        return action
    _, item, quantity = max(candidates)
    action["market"].append(["SELL", item, min(quantity, _PRESSURE_LIMITS[item])])
    return action
...
def agent(obs, configuration=None):
    try:
        ...
        return _align_hands(
            _pressure_liquidation(obs, action, step, configuration), obs
        )
    except Exception:
