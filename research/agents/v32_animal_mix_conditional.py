...
def _opponent_animal_count(obs):
    seat = _seat(obs)
...
def _mix_active(obs, step):
    seat = _seat(obs)
    state = _MIX_STATE[seat]
    if step == 0 or step < int(state.get("last_step", -1)):
        state.clear()
    state["last_step"] = step
    if step == 192:
        state["active"] = _opponent_animal_count(obs) >= 9
    return bool(state.get("active", False))


def _apply_conditional_mix(obs, action, step):
    if not _mix_active(obs, step):
        return action
    if step == 192:
...
def agent(obs, configuration=None):
    try:
        ...
        return _align_hands(_apply_conditional_mix(obs, action, step), obs)
    except Exception:
